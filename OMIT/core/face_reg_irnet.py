...
def inference(weight, name, img):
    ...
    net = get_model(name, fp16=False).to("cuda")
...
if __name__ == "__main__":
    ...
    feats1 = inference(args.weight, args.network, args.img1)
    feats2 = inference(args.weight, args.network, args.img2)
    print(torch.mean(torch.abs(feats2 - feats1)))
    ...
    print(score)#0.9028844#0.93699044#0.93447274
    # input_img_path = r"C:\Users\deng\Desktop\gen_ir1_0.8752"
    ...
    #         img2=os.path.join(r"C:\Users\deng\Desktop\rgb1",file)
    #         feats1=inference(args.weight, args.network, img1)
    #         feats2=inference(args.weight, args.network, img2)
    #         p1 = feats1 / np.sqrt(np.sum(feats1 ** 2, -1, keepdims=True))
    #         p2 = feats2 / np.sqrt(np.sum(feats2 ** 2, -1, keepdims=True))
    #         similarity_score = np.sum(p1 * p2, -1)
    #         score = similarity_score.flatten()
    #         a+=score
    #         f.write(file + "\t" + str((score+1)/2) + "\n")
    # print(a/18900)
