    ...
    def train(self, loaders):
        ...
        for i in range(args.resume_iter, args.total_iters):
            ...
            d_loss, d_losses_latent = compute_d_loss(
                nets, args, x_real, y_org, y_trg,self.network_identity,  x_ref=x_ref, masks=masks)
            self._reset_grad()
            d_loss.backward()
            ...
            g_loss, g_losses_latent = compute_g_loss(
                nets, args, x_real, y_org, y_trg, self.network_identity,self.criterionL2,self.cri_perceptual,x_refs=[x_ref, x_ref2], masks=masks)
            self._reset_grad()
            g_loss.backward()
            optims.generator.step()
            optims.style_encoder.step()
            # mapping_loss=compute_mapping_loss(
            #     nets, args, x_real, y_org, y_trg, self.network_identity, self.criterionL2, self.cri_perceptual,
            #     x_refs=[x_ref, x_ref2], masks=masks)
            # self._reset_grad()
...
###################no_cycle
# def compute_d_loss(nets, args, x_real, y_org, y_trg,network_identity, z_trg=None, x_ref=None, masks=None):
...
#                        fake=loss_fake.item(),
#                        reg=loss_reg.item())
# def compute_g_loss(nets, args, x_real, y_org, y_trg,network_identity,criterionL2,cri_perceptual,z_trgs=None, x_refs=None, masks=None):
#     ZH_input = x_refs[0]
#     face_vector_real=network_identity(x_real).detach()
#     s_trg = nets.style_encoder(x_real, y_trg)
#     x_fake = nets.generator(x_real, s_trg, masks=masks)
#     face_vector_zh_real = network_identity(ZH_input).detach()
#     s_trg2 = nets.style_encoder(ZH_input, y_trg)
#     x_fake2 = nets.generator(ZH_input, s_trg2, masks=masks)
#     loss_cyc = torch.mean(torch.abs(x_fake2 - ZH_input))
#     out = nets.discriminator(x_fake, y_trg)
#     loss_adv = adv_loss(out, 1)
#     face_vector_fake= network_identity(x_fake)
#     loss_id = criterionL2(face_vector_zh_real, face_vector_fake)
#     s_trg3 = nets.style_encoder(x_fake, y_trg)
#     loss_sty = torch.mean(torch.abs(s_trg - s_trg3))
#     loss = loss_adv + loss_cyc  + loss_id * 2 +loss_sty #+ loss_sty*2+loss_sty2*2#+loss_vgg*0.05+ loss_cyc+loss_vgg_style*0.05#+ args.lambda_cyc * loss_cyc       #+ loss_id*(1.0-args.lambda_ds)+loss_vgg*0.01#(1.1-args.lambda_ds)*
...
def compute_g_loss(nets, args, x_real, y_org, y_trg,network_identity,criterionL2,cri_perceptual,z_trgs=None, x_refs=None, masks=None):
    ...
    face_vector_fake= network_identity(x_fake)
    loss_id = criterionL2(face_vector_zh_real, face_vector_fake)
    s_trg3 = nets.style_encoder(x_fake, y_org)
    x_fake2 = nets.generator(x_fake, s_trg3, masks=masks)
