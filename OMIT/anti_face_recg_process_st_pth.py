...
if __name__ == "__main__":
    ...
    net.eval()
##########单个
    # img1 = r"D:\HFR-BUAAVISNIR\150\14.bmp"
    # img2=r"D:\HFR-BUAAVISNIR\150\2.bmp"
    #
    # feats1=inference(net, img1)
    # feats2=inference(net, img2)
    # l=torch.mean(torch.abs(torch.from_numpy(inference(net, img1)) - torch.from_numpy(inference(net, img2))))
    # p1 = feats1 / np.sqrt(np.sum(feats1 ** 2, -1, keepdims=True))
    # p2 = feats2 / np.sqrt(np.sum(feats2 ** 2, -1, keepdims=True))
    # similarity_score = np.sum(p1 * p2, -1)
    # score = similarity_score.flatten()
...
#########多个
    ...
    print(a/b)
#############同一张
    # img_list=os.listdir(r"D:\train\train\ir")
    # random.shuffle(img_list)
    # file1=open(r"D:\train\score.txt","w")
    # for i in tqdm.tqdm(img_list):
    #     img_path=os.path.join(r"D:\train\train\ir",i)
    ...
    #     similarity_score = np.sum(p1 * p2, -1)
    #     score = similarity_score.flatten()
    #     file1.write(img_path+"\t"+str((score+1)/2)+"\n")
        # print(l,(score+1)/2)